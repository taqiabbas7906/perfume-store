...
interface Props {
  initial?: EditableCollection | null
  onClose: () => void
  onSaved: () => void
...
export default function NewCollectionModal({
  ...
  const [isLimitedEdition, setLimited] = useState(
    initial?.isLimitedEdition ?? false,
  )
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    else if (isEdit) body.image = null

    if (!isEdit) body.products = []

    setSaving(true)
    try {
      const res = await authFetch(
        isEdit
          ? `/api/admin/collections/${initial!._id}`
      ...
      if (!res.ok || !data.success) {
        throw new Error(
          data.error ||
            (isEdit ? 'Failed to update collection' : 'Failed to create collection'),
        )
      }
      onSaved()
    } catch (err) {
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Rare and exclusive scents from artisanal perfume houses..."
              className="w-full bg-paper-50 border border-paper-300 px-3 py-2 text-sm text-charcoal-900 rounded focus:outline-none focus:border-gold-400"
            />
          </div>
